...
def Validar_ingreso_puntuaciones(numero_a_validar: str) -> bool:
    retorno = True
...
def verificar_puntuacion(numero:str, mensaje_reingreso:str, mensaje_error:str) -> int|float:
    while Validar_ingreso_puntuaciones(numero,) == False:
        print(f"{mensaje_error}")
...
def cargar_puntuaciones(matriz_puntuaciones:list,array_nombres:list) -> bool:
    retorno = False
    if type(matriz_puntuaciones) == list and len(matriz_puntuaciones) > 0 and type(array_nombres) == list and len(array_nombres) > 0:
        for fil in range(len(matriz_puntuaciones)):
            if type(matriz_puntuaciones[fil]) == list:
                print(f"Cargando puntajes participante: {fil + 1}")
                for col in range(len(matriz_puntuaciones[fil])):
                    puntuacion_juez = input(f"Ingrese la puntuacion del juez {col + 1}: ")
                    puntuacion_valida = verificar_puntuacion (puntuacion_juez,f"Reingrese la puntuacion del juez {col + 1}: ", "Puntuacion no valida (0-10)" )
                    matriz_puntuaciones[fil][col] = puntuacion_valida
                    retorno = True
